...
if platform == 'android':
    ...
    def on_permissions_granted(permissions, grants):
        ...
        try:
            ...
            # 权限获取后延迟初始化MQTT，避免启动卡顿
            Clock.schedule_once(lambda dt: Esp32MobileApp.get_running_app()._init_mqtt_client(), 1.0)
        except Exception as e:
            print(f"权限授予后初始化数据库失败：{e}")
            from kivymd.toast import toast
            toast(f"初始化失败：{str(e)[:20]}")
else:
    # 非安卓端空实现，避免调用报错
    def request_app_permissions(*args):
        pass
...
class Esp32MobileApp(MDApp):
    ...
    def _update_recv_data(self, content):
        ...
        try:
            # 确保在主线程更新UI（Kivy要求）
            Clock.schedule_once(lambda dt: self._safe_update_ui(content), 0)
        except Exception as e:
            print(f"调度UI更新失败：{e}")

    def _safe_update_ui(self, content):
        """线程安全的UI更新（核心容错）"""
        try:
            # 1. 写入全局日志（手机日志页面可见）
            from app_ui_pages import add_global_log
        ...
        try:
            ...
            if "我的个人中心" in page_texts:
                ...
                if hasattr(self, 'page_container') and self.page_container:
                    # 清空容器前先判空，避免重复清空报错
                    if self.page_container.children:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if platform == 'android':
            from kivymd.toast import toast
            toast(error_msg[:20])  # 限制长度避免toast显示异常
